...
#include "type_ext.h"
ostream & operator << (ostream & os, const PORT_ID &pid)
{
    os << "unit:" << pid.unit << ", index: " << pid.index;
    return os;
}

#include "qual.h"
ostream & operator << (ostream & os, const QUAL::MATCH &match)
{
    dump_container(os, match.value);
    os << "/";
    dump_container(os, match.mask);
    return os;
}

ostream & operator << (ostream & os, const QUAL::PDF_RULE &pdf_rule)
{
    os << "PDF:" << pdf_rule.first << ", MATCH: " << pdf_rule.second;
    return os;
}
